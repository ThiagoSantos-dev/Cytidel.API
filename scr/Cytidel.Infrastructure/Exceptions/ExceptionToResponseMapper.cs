...
using Omatka;
using Omatka.WebApi.Exceptions;
using System.Collections.Concurrent;
using System.Net;

namespace Cytidel.Infrastructure.Exceptions;

//mapper exceptions to a global layer.
internal sealed class ExceptionToResponseMapper : IExceptionToResponseMapper
{
    ...
    public ExceptionResponse Map(Exception exception)
        => exception switch
        {
            ...
            , HttpStatusCode.BadRequest),
            InfraException ex => new ExceptionResponse(new { code = GetCode(ex), reason = ex.Message },
                HttpStatusCode.BadRequest),
            _ => new ExceptionResponse(new { code = "error", reason = "There was an error." },
                HttpStatusCode.BadRequest)
        };
    //retrieve the code message from the exception
